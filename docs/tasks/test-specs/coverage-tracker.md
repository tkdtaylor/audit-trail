...
| 017 | Rotation runtime surface | [017-rotation-runtime-surface-test-spec.md](017-rotation-runtime-surface-test-spec.md) | TC-017-01..06 implemented + passing (`rotation_runtime_test.go`). CLI `rotate` command + IPC `{"op":"rotate"}` wired. `BuildCheckpointPayload` fixed to use `verifyFullChain` for multi-segment logs. | 🟡 Code merged | L6: live CLI `rotate` -> `{"rotated":true,"segment":"audit.log.001",...}`; below-threshold -> `{"rotated":false}`; `verify` -> `{"valid":true,...}`; live IPC `{"op":"rotate"}` -> `{"rotated":true,...}`; `emit` (seq 3), `verify`, `ping`, `checkpoint_create` (tree_size:4, global cumulative head) all unchanged; `{"op":"rotate"}` without config -> `{"error":{"code":"rotation_not_configured",...}}`; `make check` -> `ok github.com/tkdtaylor/audit-trail`; `make fitness` -> `fitness: all wired checks passed`; `go test -race ./...` -> `ok`. |
