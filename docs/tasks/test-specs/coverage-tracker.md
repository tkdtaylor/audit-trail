...
| 015 | Segment and manifest core | [015-segment-and-manifest-core-test-spec.md](015-segment-and-manifest-core-test-spec.md) | TC-015-01..08 implemented + passing (segment_test.go); `make check` + `make fitness` green | 🟡 Code merged | L3: `make check` (`ok github.com/tkdtaylor/audit-trail`) + `go build ./...` clean; `make fitness` "fitness: all wired checks passed"; `-race` clean on Rotate/Emit isolation test. data-model.md updated (segment/manifest schemas + seam invariant). Awaiting spec-verifier + L5/L6. |
